...
void CRichEditUI::DoEvent(TEventUI& event)
{
   if( !IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND ) {
	...
	{  
		...
		//GetManager()->SendNotify(this, DUI_MSGTYPE_RBTN_RICHEDIT, event.wParam, event.lParam);  
...
LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    ...
    if((uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) {
        ...
        else if( uMsg == WM_LBUTTONDOWN || uMsg == WM_LBUTTONDBLCLK || uMsg == WM_RBUTTONUP) {
...
BOOL CRichEditUI::CanPaste(UINT nFormat/* = 0*/)
{
	...
	if (NULL == m_pTwh->GetTextServices())
		return FALSE;

	HRESULT lRes = 0;
	m_pTwh->GetTextServices()->TxSendMessage(EM_CANPASTE, nFormat, 0L, &lRes);
	return (BOOL)lRes;
}
...
void CRichEditUI::PasteSpecial(UINT uClipFormat, DWORD dwAspect, HMETAFILE hMF)
{
	...
	m_pTwh->GetTextServices()->TxSendMessage(EM_PASTESPECIAL, uClipFormat, (LPARAM)&reps, NULL);
}
